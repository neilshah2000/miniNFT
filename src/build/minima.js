 ...
 */
const MINIMA_SERVER_LISTEN = [];
const MINIMA_USER_LISTEN = [];
/**
    ...
    init: function (callback) {
        ...
        if (!Minima.debug) {
            if (window.location.protocol.startsWith("http")) {
                ...
                const rpcPort = +window.location.port - 2;
                const wsPort = +window.location.port - 1;
 ...
 * @returns
 */
function httpGetAsync(theUrl, callback, logenabled = false) {
    const xmlHttp = new XMLHttpRequest();
    xmlHttp.onreadystatechange = function () {
        if (xmlHttp.readyState == 4 && xmlHttp.status == 200) {
            //Always a JSON ..
...
//# sourceMappingURL=minima.js.map